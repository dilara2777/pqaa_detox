...
//Expect el to have a specific text, label or ID
When('I expect the Member List header test to be {string}', async (text)=>{
...
//Expect el exist/not exist
//iphone, will fail on Android because Android is not rendering not visible elements, not recognised flat list, must be vertical or horizontal in hierarchy, not a flat list
When('I verify that the last image in the row exist', async ()=>{
...
//Expect el visible/not visible
When('I verify that the first image in the row is visible', async ()=>{
    await expect(element(by.id('image-europe-0'))).toBeVisible();
})

Then('I verify that the last image in the row is not visible', async ()=>{
